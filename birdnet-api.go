		...
		return func(ctx echo.Context) error {
			...
			if err != nil {
				...
				if errors.As(err, &he) {
					...
					// status set yet â€” Echo's error handler runs after this
...
func (c *Controller) newErrorResponse(err error, message string, code int) *ErrorResponse {
	...
	// Only expose raw err.Error() in debug mode â€” it can contain internal
...
func (c *Controller) handleErrorInternal(ctx echo.Context, err error, message string, code int, errorKey string, errorParams map[string]any) error {
	...
	// 4xx errors are client mistakes (bad input, not found) â€” not bugs â€” and are excluded.
