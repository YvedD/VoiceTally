...
> **âš ï¸ Custom Deployments:** The instructions above apply specifically to installations done via the official `install.sh` script. Custom Docker Compose setups or manual binary deployments may handle timezone configuration differently.

> **ðŸ’¡ System vs Application Timezone:** BirdNET-Go can use a different timezone than your system. This is useful if you want your system in one timezone but want BirdNET-Go to record detections in another (e.g., UTC for standardized scientific data).

> **ðŸ”„ After Updates:** When updating BirdNET-Go using the `install.sh` script, your timezone settings are preserved automatically as of recent versions.
...
1. **Always Exclude Species** (Absolute Override)
   ...
   - Configured via: Settings â†’ Species â†’ "Always Exclude Species"
...
2. **Always Include Species** (Absolute Override)
   ...
   - Configured via: Settings â†’ Species â†’ "Always Include Species"
...
graph TD
    ...
    K -->|Count = 1| L[Level 1:<br/>Threshold Ã— 0.75]
    K -->|Count = 2| M[Level 2:<br/>Threshold Ã— 0.5]
    K -->|Count â‰¥ 3| N[Level 3:<br/>Threshold Ã— 0.25]
...
[Deep Detection](BirdNETâ€Go-Guide#deep-detection) uses the `overlap` setting to require multiple detections of the same species within a configurable detection window before accepting it, significantly reducing false positives.
...
- overlap: 0.0  â†’ segmentLength: 3.0  â†’ minDetections: 1  (standard mode)
- overlap: 1.5  â†’ segmentLength: 1.5  â†’ minDetections: 2
- overlap: 2.4  â†’ segmentLength: 0.6  â†’ minDetections: 4  (recommended)
- overlap: 2.7  â†’ segmentLength: 0.3  â†’ minDetections: 8  (deep detection)
- overlap: 2.9  â†’ segmentLength: 0.1  â†’ minDetections: 24 (very strict)

With longer detection window (30s clip, 0s pre-capture = 30s window):
- overlap: 2.4  â†’ minDetections: 10 (2x baseline)
- overlap: 2.7  â†’ minDetections: 20 (2x baseline)
```
...
- **Problem Solved**: The BirdNET AI model frequently confuses dog barks with certain bird calls, especially owl vocalizations (very common: dog bark â†’ Eurasian Eagle-Owl)
...
**Tip**: If the range filter results don't match your expectations for specific species, you can override them using the **Species Settings** in the web interface (Settings â†’ Species) rather than adjusting the global threshold.
...
- Navigate to **Settings â†’ Species** in the web interface
...
- **Solution**: Add the species to the "Always Include Species" list via **Settings â†’ Species** in the web interface, or lower the `threshold` value if many species are missing
...
- **Solution**: Add these to the "Always Exclude Species" list via **Settings â†’ Species**
...
4. **Configure BirdNET-Go**:
   ...
   - In your BirdNET-Go web interface, go to Settings â†’ Security
...
graph TD
    ...
    B -->|Yes| C[â±ï¸ Start 15-Second<br/>Counting Window<br/>Count = 1]
    B -->|No| D[ðŸ“ˆ Increment Counter<br/>Count = Count + 1]

    C --> E[ðŸŽ§ Continue Listening<br/>for More Audio]
    D --> E

    E --> F{Same Species<br/>Detected Again?}
    F -->|Yes| G[â¬†ï¸ Add to Count]
    ...
    I -->|No| J[âŒ Rejected<br/>False Positive]
    I -->|Yes| K{Final Safety<br/>Checks Pass?}

    K -->|No| L[âŒ Blocked by<br/>Privacy/Dog Filter]
    K -->|Yes| M[âœ… Detection Approved<br/>Saved to Database]

    J --> N[ðŸ—‚ï¸ Log: Not enough matches]
    L --> O[ðŸ—‚ï¸ Log: Blocked by filter]
    M --> P[ðŸ—‚ï¸ Log: Species confirmed]
```
...
4. **Variable Threshold**: The number of required detections scales with the overlap setting:
   ...
   Examples:
   - overlap: 0.0 â†’ 1 detection required (standard mode)
   - overlap: 2.4 â†’ 5 detections required
   - overlap: 2.7 â†’ 10 detections required (typical deep detection)
   - overlap: 2.9 â†’ 30 detections required (very strict)
   ```
...
- **How to Use:**
  ...
  4.  Click the play icon (â–¶ï¸) next to the source name.
  5.  Audio playback will begin using your browser's audio capabilities.
  6.  Click the stop icon (â¹ï¸) to end the stream.
...
1. **1/3rd Octave Band Filtering**:
   ...
   - Q factor calculation: `Q = f_center / (f_high - f_low)` â‰ˆ 4.318 for 1/3rd octave bands
...
3. **Data Aggregation**:
   - Two-stage aggregation: 1-second measurements â†’ 10-second statistics
...
5. Copy the webhook URL â€” it looks like: `https://discord.com/api/webhooks/1234567890/abcDEFghiJKLmnoPQRstu`
...
â†’ discord://abcDEFghiJKLmnoPQRstu@1234567890
```

**Step 3: Configure in BirdNET-Go**

Via the web UI:
1. Go to **Settings** â†’ **Notifications** â†’ **Channels**
...
- **"Test Provider" works but real detections don't?** Check that the daylight filter isn't blocking detections for nocturnal species during daytime (Settings â†’ Detection Filters).
...
```yaml
# âŒ NEVER do this (hardcoded secret)
token: "123456789:ABCdefGHIjklMNOpqrsTUVwxyz"

# âœ… DO this (environment variable)
token: "${TELEGRAM_BOT_TOKEN}"

# âœ… OR this (secret file)
...
##### ðŸŒŸ **New Species** (Lifetime First)
...
##### ðŸŒ¿ **New This Season** (Seasonal First)
...
1. **ðŸŒŸ New Species** (highest priority - truly first-time visitors)
2. **ðŸ“… New This Year** (medium priority - annual firsts)
3. **ðŸŒ¿ New This Season** (lowest priority - seasonal appearances)
...
- **New Species Window** (`newSpeciesWindowDays`): How many days to display the ðŸŒŸ "New Species" badge after a bird is detected for the first time ever at your location (default: `7` days)
...
- **Badge Display Window** (`yearlyTracking.windowDays`): How many days to show the ðŸ“… "New This Year" badge (default: `7` days)
...
- **Badge Display Window** (`seasonalTracking.windowDays`): How many days to show the ðŸŒ¿ "New This Season" badge (default: `7` days)
- **Season Definitions** (`seasonalTracking.seasons`): Customize when each season begins
  - **Automatic Hemisphere Detection**: If you don't specify custom seasons, the system automatically adjusts based on your latitude:
    - Northern Hemisphere (latitude > 10Â°): Spring starts March 20, Summer June 21, etc.
    - Southern Hemisphere (latitude < -10Â°): Seasons are shifted by 6 months
    - Equatorial regions (latitude -10Â° to 10Â°): Uses wet/dry season patterns
    ...
    try:
        ...
        for event in client.events():
            if event.event == 'connected':
                ...
                print(f"ðŸ¦ {detection['commonName']} detected!")
                ...
                data = json.loads(event.data)
                print(f"ðŸ’“ Heartbeat - {data['clients']} clients connected")

    except KeyboardInterrupt:
        print("\nðŸ‘‹ Disconnecting from stream...")
...
class BirdNetGoClient {
  ...
  connect() {
    ...
    console.log("ðŸ”— Connecting to BirdNET-Go detection stream...");
  }

  onDetection(detection) {
    console.log(`ðŸ¦ ${detection.commonName} detected!`);
    ...
    if [[ $line == data:* ]]; then
        ...
        if command -v jq &> /dev/null; then
            ...
            if echo "$json_data" | jq -e '.commonName' &> /dev/null; then
                ...
                echo "ðŸ¦ $time: $species (${confidence})"
...
function connectWithRetry(baseUrl, maxRetries = 5) {
  ...
  function connect() {
    ...
    eventSource.onopen = function () {
      console.log("âœ… Connected to detection stream");
    ...
    eventSource.onerror = function (event) {
      if (retryCount < maxRetries) {
        ...
        console.log(
          `âŒ Connection lost. Retrying in ${delay}ms... (attempt ${retryCount}/${maxRetries})`,
        );
        ...
        console.error("âŒ Max retries reached. Please check your connection.");
      }
    };
