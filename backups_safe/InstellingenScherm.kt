...
class InstellingenScherm : AppCompatActivity() {
    ...
    companion object {
        ...
        const val PREF_BIRDNET_MIN_CONFIDENCE = "pref_birdnet_min_confidence"
        ...
        const val DEFAULT_BIRDNET_MIN_CONFIDENCE = 70
        ...
        fun getBirdNetMinConfidence(context: Context): Int {
            val prefs = context.getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
            return prefs.getInt(PREF_BIRDNET_MIN_CONFIDENCE, DEFAULT_BIRDNET_MIN_CONFIDENCE)
        }
    ...
    private fun setupBirdNetConfidencePicker() {
        val prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE)
        val npConfidence = findViewById<NumberPicker>(R.id.npBirdNetMinConfidence)
        npConfidence.minValue = 0
        npConfidence.maxValue = 100
        npConfidence.wrapSelectorWheel = false
        npConfidence.value = prefs.getInt(PREF_BIRDNET_MIN_CONFIDENCE, DEFAULT_BIRDNET_MIN_CONFIDENCE)
            .coerceIn(0, 100)
        npConfidence.setOnValueChangedListener { _, _, newVal ->
            prefs.edit { putInt(PREF_BIRDNET_MIN_CONFIDENCE, newVal) }
        }
    }
