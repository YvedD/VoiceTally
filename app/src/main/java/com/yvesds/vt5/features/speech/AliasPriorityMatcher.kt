...
object AliasPriorityMatcher {
    ...
    suspend fun match(
        ...
        while (i < tokens.size) {
            ...
            for (w in maxWindow downTo 1) {
                ...
                if (window.any { isNumberToken(it) }) continue
                ...
                if (exact != null) {
                    ...
                    if (nextIndex < tokens.size) {
                        parseAmountToken(tokens[nextIndex])?.let { amt ->
                            amount = amt
                            consumedEndExclusive = nextIndex + 1
                            i = nextIndex + 1
                        } ?: run { i = nextIndex }
                    } else i = nextIndex
    ...
    private suspend fun tryResolveWholePhraseMatch(
        ...
        val amount = parseAmountToken(tokens.last()).takeIf { tokens.size > 1 } ?: 1
        val phraseTokens = if (amount != 1) tokens.dropLast(1) else tokens
